...
(function (document, $, wordfind) {
    ...
    var WordFindGame = function () {
        ...
        var endTurn = function () {
            ...
            for (var i = 0, len = wordList.length; i < len; i++) {
                ...
                if (wordList.length === 0) {
                    CheckCompletionGame(wordList.length);
        ...
        var CheckCompletionGame = function (lengh) {
            debugger;

            var ST = d;
            console.log(d);
            var IN_H = new Date(d).getHours();
            var IN_M = new Date(d).getMinutes();
            var IN_S = new Date(d).getSeconds();

            const o = new Date();
            var ET = o;
            //var ST = document.getElementById('Txt_Date_Time').value;

            //var IN_H = new Date(ST).getHours();
            //var IN_M = new Date(ST).getMinutes();
            //var IN_S = new Date(ST).getSeconds();

            var OUT_H = new Date(o).getHours();
            var OUT_M = new Date(o).getMinutes();
            var OUT_S = new Date(o).getSeconds();
            ...
            if (lengh == 0) {
                var customerID = Global_CourseID;
                var firstname = "John";
                $.ajax({
                    type: 'POST',
                    dataType: 'JSON',
                    url: '/Competition/PuzzleSave',
                    async: false, // Make the request synchronous
                    data: { CourseID: customerID, fname: firstname, PractID: GetPractID, startTime: d, endTime: ET, TimeDiff: HrsDiff, UserAnswer: 1 },
                    success: function (response) {
                        debugger;
                        if (JSON.parse(response)[0] == "1") {
                            alert("Congrats");
                            setTimeout(function () {
                                // Assume response[1] contains the updated DataTable in JSON format
                                var updatedData = JSON.parse(response)[1];
                                // Use the updated data to display the next puzzle
                                btn_Next(updatedData);
                            }, 2000);
                        } else {
                            alert("Congrats! " + JSON.parse(response)[1]);
                            setTimeout(function () {
                                window.location = '/Home/WordCompetition';
                            }, 2000);
                        }
                    },
        ...
        return {
             ...
             */
            create: function (words, puzzleEl, wordsEl, options, options1) {

                debugger;
                Global_CourseID = options;
                GetPractID = options1;
